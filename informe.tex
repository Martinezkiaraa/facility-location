...
En los resultados presentados, las dos versiones asignaron únicamente una fracción de los 208 parciales. La distribución se concentra fuertemente en grupos muy separados entre sí, lo cual es coherente con el objetivo de maximizar $|d_1 - d_2|$.  
...
\subsection{Trade-off entre cantidad de parciales y dispersión}

Este comportamiento ilustra un aspecto central de la modelización en programación entera:
Maximizar la dispersión suele entrar en conflicto directo con maximizar la cantidad de exámenes programados.

Si el objetivo principal es evitar que los estudiantes tengan exámenes concentrados, el solver puede decidir no programar ciertos parciales (poniendo todas sus variables $X$ en cero) para mejorar el valor de la función objetivo. Esto sucede especialmente cuando:
\begin{itemize}
    \item un curso tiene muchos vecinos en el grafo;
    \item los días están muy acotados;
    \item las restricciones de no proximidad entre días son estrictas.
\end{itemize}

En otras palabras:
\[
\text{Más dispersión} \;\Rightarrow\; \text{menos cursos asignados.}
\]

Por el contrario:
\[
\text{Más cursos asignados} \;\Rightarrow\; \text{menos separación posible.}
\]

Este es un trade-off estructural que no se puede evitar sin
\begin{itemize}
    \item relajar la dispersión,
    \item permitir más días de examen,
    \item flexibilizar la capacidad de aulas,
    \item o incorporar penalizaciones en vez de prohibiciones estrictas.
\end{itemize}

\subsection{Conclusión}

El modelo de la Consigna 3 muestra cómo, incluso con información parcial sobre los estudiantes (sólo pares en común), es posible construir una función objetivo que promueva una asignación más equilibrada y menos concentrada de los exámenes.

Sin embargo:
\begin{itemize}
    \item la dispersión fuerte es incompatible con asignar la totalidad de los parciales;
    \item el solver tiende a sacrificar cantidad para incrementar la distancia entre cursos fuertemente conectados;
    \item y el resultado final depende sensiblemente de qué tan estrictas sean las restricciones adicionales.
\end{itemize}

Este ejercicio evidencia la importancia de definir objetivos equilibrados (multi-criterio o con pesos ajustables) para evitar que priorizar un aspecto del problema (la dispersión) produzca soluciones que deterioran otros aspectos relevantes (cantidad de parciales programados).

\section{Comparación entre la Versión 1 y la Versión 2 del modelo (Consigna 3)}

Recordemos que en la Consigna 3 analizamos dos variantes del modelo de dispersión, ambas con la misma función objetivo basada en
\[
I_{pq} \cdot |d_1 - d_2| \cdot Z_{pq,d_1,d_2},
\]
pero con distintas restricciones sobre la cercanía entre días (días iguales o consecutivos en la Versión 1, y días aún más próximos en la Versión 2).

En esta subsección nos concentramos únicamente en comparar el resultado que producen esas dos variantes.

\subsection{Distribución comparada de parciales por día}
...
La Figura~\ref{fig:comparacion-ej3} presenta un gráfico de barras comparando ambas soluciones.
\begin{figure}[H]
    \centering
